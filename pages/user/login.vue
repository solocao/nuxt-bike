<template>
  <div class="index">
    ...
    <div v-if="page === 1" class="section section-login" style="z-index: 6">
    ...
    <div class="section section-about" style="z-index: 5" :class="{'is-old': page > 1}">
...
export default {
  ...
  methods: {
    ...
    async login() {
      ...
      if (result.code === 200) {
        await this.set({ login: true, user: result.data })
        this.$router.push('/')
      } else {
        this.$Modal.confirm({
          title: '提示',
    ...
    async register() {
      ...
      if (isMobile(this.username)) {
        result = await this.post('/user/register/mobile', parms)
      } else {
        // 用户名注册
        result = await this.post('/user/register/name', parms)
      }
